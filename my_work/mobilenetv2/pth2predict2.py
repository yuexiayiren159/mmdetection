...
from typing import List, Optional, Sequence, Tuple, Union, Dict  # 加入 Dict
...
def predict_by_feat(pred_maps: Sequence[Tensor],
    ...
    # 如果 pred_maps 是 tuple 列表，提取其中的 Tensor
    featmap_sizes = [pred_map[0].shape[-2:] if isinstance(pred_map, tuple) else pred_map.shape[-2:] for pred_map in
                     pred_maps]

    # mlvl_anchors = prior_generator.grid_priors(
    #     featmap_sizes, device=pred_maps[0].device)
    mlvl_anchors = prior_generator.grid_priors(
        featmap_sizes, device=pred_maps[0])
        ...
        for i, output in enumerate(outputs):
            ...
            if isinstance(output, tuple):
                for j, item in enumerate(output):
                    print(f'Output[{i}][{j}] type: {type(item)}, shape: {getattr(item, "shape", None)}')
            elif isinstance(output, torch.Tensor):
                print(f'Output[{i}] shape: {output.shape}')

    # 调用解码函数
    # batch_img_metas 可以从 model.data_preprocessor 里面获取
    # 这里假设是从 model.data_preprocessor 的配置获取
    batch_img_metas = [{
        'ori_shape': (416, 416, 3),
        'img_shape': (416, 416, 3),
        'scale_factor': (1.0, 1.0)
    }]
    batch_img_metas = [{'scale_factor': np.array([1.0, 1.0, 1.0, 1.0])}] * len(outputs)
    detections = predict_by_feat(outputs, batch_img_metas, model.bbox_head.bbox_coder,
                                 model.bbox_head.prior_generator,
                                 model.bbox_head.num_classes,
                                 model.bbox_head.featmap_strides)
