...
# def preprocess_image(img_path, target_size=(288, 416), pad_size_divisor=32):
#     # 读取图像
#     img_bgr = cv2.imread(img_path)
#     if img_bgr is None:
...
#     resized_size = (int(original_width * scale), int(original_height * scale))
#
#     # 调整图像大小
#     img_pil = img_pil.resize(resized_size, Image.BILINEAR)
#
#     # 创建带填充的背景图像
...
#     ])
#     img_tensor = transform(background)
#
...
def preprocess_image(img_path, target_size=(288, 416), intermediate_size=(278, 416)):
    ...
    #
    # # 计算保持宽高比的缩放尺寸
    # inter_height, inter_width = intermediate_size
    # scale = min(inter_width / original_width, inter_height / original_height)
    # resized_size = (int(original_width * scale), int(original_height * scale))
    #
    # # 调整图像大小到 intermediate_size (278, 416)
    # img_pil = img_pil.resize(resized_size, Image.BILINEAR)
    #
    #
    # # 创建带填充的背景图像到 target_size (288, 416)
...
def process_yolo_outputs(outputs, img_size=(288, 416), conf_threshold=0.05, nms_threshold=0.45):
    boxes = []
    confidences = []
    class_ids = []
...
with torch.no_grad():
    ...
    #batch_img_metas[(img_path),(scale_factor(2)),(288,416),(orishape 427,416),(pad_shape288,416),(img_shape278,416)]
    # predictions = predict_by_feat(outs)
